...
(	
    ...
    `Created` INTEGER DEFAULT CURRENT_TIMESTAMP, -- Time when index creation started
...
(
    `Path` TEXT NOT NULL,   -- Machine name of the path that this column belongs to 
    `URI` TEXT NOT NULL,    -- Canonical Entity URI
    `Value` BLOB            -- JSON-encoded value for this column. See go 'value.go' for exact struct.
);

CREATE INDEX `wca_path` ON `wca_columns`
-- lookup all entities for a given path
(
    `Path`
);                  
CREATE INDEX `wca_uri` ON `wca_columns`
-- find value for specific uri
(
    `Path`,
    `URI`
);
...
CREATE UNIQUE INDEX `wca_canonical` ON `wca_sameas`
-- Lookup a canonical URI
(
    `Alias`
);      
CREATE UNIQUE INDEX `wca_aliasof` ON `wca_sameas`
-- Find aliases of a specific URI
(
    `Canonical`
);
